...
if (empty($_REQUEST['action']))
{
	...
	$sql = "select * from fjr_shops order by id";
	...
	$shopinfo = '
<tr>
<td colspan="2" align="center" class="row2"><span class="gen"><b>Global Shop Settings</b></span></td>
</tr>
<tr>
...
<tr>
<td colspan="2" align="center" class="row2"><span class="gen"><b>Edit Player Inventories</b></span></td>
</tr>
...
<tr>
<td colspan="2" align="center" class="row2"><span class="gen"><b>Shop Modifications</b></span></td>
</tr>
<tr>
...
<tr>
<td class="row2"><form method="post" action="'.append_sid("admin_shop.$phpEx").'"><span class="gensmall"><b>Special Shop</b></span></td><td class="row2"><input type="hidden" name="action" value="editspecialshop"><input type="submit" class="liteoption" value="Edit"></td></form>
</tr>';
...
elseif ($_REQUEST['action'] == "createshop")
{
	...
	else
	{
		...
		$sql = "select * from fjr_shops where shopname='{$_REQUEST['shopname']}'";
		if ( !($result = $db->sql_query($sql)) )
		{
		...
		$sql = "insert into fjr_shops (shopname, shoptype, restocktime, restockamount) values('{$_REQUEST['shopname']}', '{$_REQUEST['shoptype']}', '$restocktime', '$restockamount')";
		if ( !($result = $db->sql_query($sql)) )
		{
...
elseif ($_REQUEST['action'] == "updateshop")
{
	...
	{
		$sql = "select * from fjr_shops where id='{$_REQUEST['shopid']}'";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error());
		}
		...
		$sql = "update fjr_shops set shopname='{$_REQUEST['name']}', shoptype='{$_REQUEST['shoptype']}', restocktime='{$_REQUEST['restocktime']}', restockamount='{$_REQUEST['restockamount']}' where id='{$_REQUEST['shopid']}'";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_MESSAGE, 'Fatal Error Updating Shop');
		}
		$sql = "update fjr_shopitems set shop='{$_REQUEST['name']}' where shop='" . addslashes($row['shopname']) . "'";
		if ( !($result = $db->sql_query($sql)) )
		{
...
elseif ($_REQUEST['action'] == "additem")
{
	...
	{
		$sql = "select shopname from fjr_shops where id='{$_REQUEST['shopid']}'";
		if ( !($result = $db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, "Fatal Error: ".mysql_error()); }
		$row = mysql_fetch_array($result);

		$sql = "select * from fjr_shopitems where name='{$_REQUEST['item']}'";
		if ( !($result = $db->sql_query($sql)) )
		if (mysql_num_rows($result) > 0)
		{
			message_die(GENERAL_MESSAGE, 'This item already exists!');
		}
		$sql = "insert into fjr_shopitems (name, shop, sdesc, ldesc, cost, stock, maxstock, sold, accessforum ) values('{$_REQUEST['item']}', '" . addslashes($row['shopname']). "', '{$_REQUEST['shortdesc']}', '{$_REQUEST['longdesc']}', '{$_REQUEST['price']}', '{$_REQUEST['stock']}', '{$_REQUEST['maxstock']}', '0', '{$_REQUEST['accessforum']}')";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error());
		}
...
{
	$sql = "select * from fjr_shopitems where id='{$_REQUEST['itemid']}'";
	if ( !($result = $db->sql_query($sql)) )
	{
	...
	if ((!is_null($_REQUEST['shop'])) && (strlen($_REQUEST['shop']) > 2) && ($_REQUEST['shop'] != $row['shop'])) { 
		$sql = "select * from fjr_shops where shopname='{$_REQUEST['shop']}'";
		if ( !($result = $db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
	...
	$sql = "update `fjr_shopitems` set name='{$_REQUEST['name']}', shop='{$_REQUEST['shop']}', sdesc='{$_REQUEST['shortdesc']}', ldesc='{$_REQUEST['longdesc']}', cost='{$_REQUEST['price']}', stock='{$_REQUEST['stock']}', maxstock='{$_REQUEST['maxstock']}', sold='{$_REQUEST['sold']}', accessforum='{$_REQUEST['accessforum']}' where id='{$_REQUEST['itemid']}'";
	if ( !$db->sql_query($sql) )
  	{
  		message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error());
  	}
	$sql = "select id from fjr_shops where shopname='{$_REQUEST['shop']}'";
	if ( !$result = $db->sql_query($sql) )
  	{
  		message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error());
  	}
...
elseif ($_REQUEST['action'] == "deleteshop")
{
	...
	$sql = "select * from fjr_shopitems where shop='" . addslashes($row['shopname']) . "'";
	if ( !($result = $db->sql_query($sql)) )
	{
	...
	for ($xe = 0; $xe < mysql_num_rows($result); $xe++)
	{
		...
		$sql = "delete from fjr_shopitems where id='{$irow['id']}'";
		if ( !($db->sql_query($sql)) )
		{
			message_die(GENERAL_MESSAGE, 'Fatal Error Deleting Item Entry!');
		}
	}
	$sql = "delete from fjr_shops where shopname='" . addslashes($row['shopname']) . "'";
...
elseif ($_REQUEST['action'] == "deleteitem")
{
	...
	$sql = "select id from fjr_shops where shopname='{$row['shop']}'";
  	if ( !($result = $db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
	$srow = mysql_fetch_array($result);	
	...
	$sql = "delete from fjr_shopitems where id='{$_REQUEST['itemid']}'";
  	if ( !($result = $db->sql_query($sql)) )
  	{
...
elseif ($_REQUEST['action'] == "editinventory")
{
	...
	//get all items
	$isql = "select name from fjr_shopitems";
...
elseif ($_REQUEST['action'] == "updateinv")
{
	...
	if (!empty($_REQUEST['itemname']))
	{
		...
		$sql = "select * from fjr_shopitems where name='{$_REQUEST['itemname']}'";
		if ( !($result = $db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
		if (mysql_num_rows($result) < 1) { message_die(GENERAL_MESSAGE, 'No such item exists!'); }
	}
