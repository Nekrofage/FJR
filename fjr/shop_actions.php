...
*                            ------------------- 
*   Version              : 2.6.0 
*   released             : Saturday, February 8th, 2003 
...
/*************************************************************************** 
* 
*   Original shop_give code (c) copyright 2003, Moogie. 
*   Shop and shop_actions code (c) copyright 2003, Zarath/IcE-RaiN
* 
...
{ 
	...
	if (strtolower($userdata['username']) == strtolower($username)) { message_die(GENERAL_MESSAGE, 'What is the point in giving your <b>'.$itemname.'</b> to yourself?'); } 
	...
	$shoplocation = ' -> <a href="shop.php?action=inventory&searchid='.$userdata['user_id'].'" class="nav">Inventory</a> -> <a href="shop_actions.php?action=give" class="nav">Give</a> -> <a href="shop_actions.php?action=give" class="nav">Confirm Trade</a>'; 
	$shoptablerows = 1;
	$shopaction = '<tr><td class="row1" align="center"><BR><span class="gensmall">Are you sure you want to give the <b>'.$_REQUEST['itemname'].'</b> to <b>'.$_REQUEST['username'].'</b>?</span></td></tr>'; 
	$shopinforow = '<form name="post" method="post" action="shop_actions.php?action=giveitem"><input type="hidden" name="itemname" value="'.$_REQUEST['itemname'].'"><input type="hidden" name="username" value="'.$_REQUEST['username'].'"><tr><td class="row2" align="center"><input type="submit" value="Yes" class="liteoption"> <input type="button" value="No" onclick="document.location=\'shop_actions.php\'"></td></tr></form>'; 
} 
...
{ 
	...
	if (strtolower($userdata['username']) == strtolower($_REQUEST['username'])) { message_die(GENERAL_MESSAGE, 'What is the point in giving your <b>'.$_REQUEST['itemname'].'</b> to yourself?'); } 
	...
	$shoplocation = ' -> <a href="shop.php?action=inventory&searchid='.$userdata['user_id'].'" class="nav">Inventory</a> -> <a href="shop_actions.php?action=give" class="nav">Give</a> -> <a href="shop_actions.php?action=give" class="nav">Confirm Trade</a> -> <a href="shop_actions.php?action=give" class="nav">Trade Completed</a>'; 
	$shoptablerows = 1;
	$shopaction = '<tr><td class="row1" align="center"><BR><span class="gensmall"><b>'.$_REQUEST['username'].'</b> received the <b>'.$_REQUEST['itemname'].'</b>.</span><P></td></tr>'; 
} 
...
elseif ($_REQUEST['action'] == "trade")
{
	...
	if ((!(empty($_REQUEST['username']))) && ($_REQUEST['username'] != $userdata['username']))
	{
		...
		else
		{
			...
			{
				$shopaction .= '<tr><td class="row2" colspan="2" align="center"><span class="gensmall"><font color="red"><b>'.str_replace("&lt;br /&gt;", "<br />", htmlspecialchars($errormessage)).'</b></font></span></td></tr>';
			}
			$shopaction .= '<form method="post" action="shop_actions.php?action=trade">
			'.$hiddenfields.'
			...
			<form method="post" action="shop_actions.php?action=trade">
			'.$hiddenfields.'
			...
			<tr><td class="row2" valign="top"><span class="gen"><b>You\'re currently offering: </b></td><td class="row2"><span class="gensmall">'.$tradingitems.' and '.$tradegold.' '.$board_config['points_name'].'</span></td></tr>
			...
			<tr><td class="row2" valign="top"><span class="gen"><b>You\'re currently requesting: </b></td><td class="row2"><span class="gensmall">'.$otheritemz.' and '.$othergold.' '.$board_config['points_name'].'</span></td></tr>
			<form method="post" action="shop_actions.php?action=confirmtrade">
			'.$hiddenfields.'
	...
	else
	{
		...
		$shopinforow = '<form name="post" method="post" action="shop_actions.php?action=trade"><tr><td class="row2" width="50%"><span class="gensmall">Trade With:</span></td><td class="row1"><input type="text" class="post" name="username"> <input type="submit" name="usersubmit" value="Find Username" class="liteoption" onClick="window.open(\'./search.php?mode=searchuser\', \'_phpbbsearch\', \'HEIGHT=250,resizable=yes,WIDTH=400\');return false;" /></select></td></tr><tr><td class="row2" colspan="2" align="center"><input type="submit" value="Execute" class="liteoption"></td></tr></form>'; 
...
elseif ($_REQUEST['action'] == "confirmtrade" || $_REQUEST['action'] == "proposetrade")
{
	...
	if ((!empty($_REQUEST['username']) && strlen($otheruser['username']) > 2 && $_REQUEST['username'] != $userdata['username']) && (!empty($_REQUEST['tradeitems']) || !empty($_REQUEST['tradegold'])) && (!empty($_REQUEST['otheritems']) || !empty($_REQUEST['othergold'])))
	{
		...
		if ($_REQUEST['action'] == "confirmtrade")
		{
			...
			$shopaction .= '
			<tr><td class="row2" colspan="2" align="center"><span class="gensmall"><b>Remember, once you propose a trade you can\'t retract it!</b></span></td></tr>
			...
			<form method="post" action="shop_actions.php?action=proposetrade">
			'.$hiddenfields.'
...
elseif (($_REQUEST['action'] == "accepttrade") || ($_REQUEST['action'] == "rejecttrade"))
{
	...
	else 
	{
		...
		if ($_REQUEST['action'] == "accepttrade")
		{
			...
			$sql = "update " . USERS_TABLE . " set user_points = user_points - $tradearray[2] where user_id='{$tradearray[0]}'";
			if ( !($db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
			$sql = "update " . USERS_TABLE . " set user_points = user_points + $tradearray[2] where user_id='{$userdata['user_id']}'";
			if ( !($db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }


			//take tradee's points & add them to trader
			$sql = "update " . USERS_TABLE . " set user_points = user_points - $tradearray[4] where user_id='{$userdata['user_id']}'";
			if ( !($db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
			$sql = "update " . USERS_TABLE . " set user_points = user_points + $tradearray[4] where user_id='{$tradearray[0]}'";
			if ( !($db->sql_query($sql)) ) { message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error()); }
			...
			$shopaction .= '<tr><td class="row2" colspan="2" align="center"><span class="gensmall"><b>You have accepted the proposed trade by '.$row['username'].'.</b></span></td></tr>';
			$shoplocation = ' -> <a href="shop.php?action=inventory&searchid='.$userdata['user_id'].'" class="nav">Inventory</a> -> <a href="shop_actions.php?action=trade" class="nav">Trade Accepted</a>'; 
		...
		elseif ($_REQUEST['action'] == "rejecttrade")
		{
			...
			$shopaction .= '<tr><td class="row2" colspan="2" align="center"><span class="gensmall"><b>You have rejected the proposed trade by '.$row['username'].'.</b></span></td></tr>';
			$shoplocation = ' -> <a href="shop.php?action=inventory&searchid='.$userdata['id'].'" class="nav">Inventory</a> -> <a href="shop_actions.php?action=trade" class="nav">Trade Declined</a>'; 
