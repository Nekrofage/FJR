...
if ($_REQUEST['action'] == "buy") 
{
	...
	$sql = "select * from fjr_shopitems where id='{$_REQUEST['item']}'";
	...
	$sql = "select * from fjr_shops where shopname='$checkshop' and shoptype!='special' and shoptype!='admin_only'";
	...
	$sql="update fjr_shopitems set stock='$newstock', sold='$newsold' where id='{$_REQUEST['item']}'";
	if ( !($db->sql_query($sql)) )
	{
		message_die(GENERAL_MESSAGE, 'Fatal Error: '.mysql_error());
	}
	...
	$shopinforow = '<tr><td width="2%" class="row2"><span class="gensmall"><b>Icon</b></span></td><td class="row2"><span class="gen"><b>Item&nbsp;Name</b></span></td><td class="row2"><span class="gen"><b>Description</b></span></td><td class="row2"><span class="gen"><b>Stock</b></span></td><td class="row2"><span class="gen"><b>Cost</b></span></td><td class="row2"><span class="gen"><b>Owned</b></span></td></tr>';


	if (file_exists("shop/images/".$row['name'].".jpg")) { $itemfilext = "jpg"; }
	else { $itemfilext = "gif"; }
	$shopitems = '<tr><td class="row1"><img src="shop/images/'.$row['name'].'.'.$itemfilext.'" title="'.ucfirst($row['name']).'" alt="'.$row['name'].'"></td><td class="row1"><span class="gensmall"><b>'.ucwords($row['name']).'</a><b></span></td><td class="row2"><span class="gensmall">'.ucfirst($row['ldesc']).'</span></td><td class="row2"><span class="gensmall">'.$row['stock'].'</span></td><td class="row2"><span class="gensmall">'.$row['cost'].'</span></td><td class="row2"><span class="gensmall">'.$useritemamount.'</span></td></tr>';
	$title = "Buy ".$row['name'];
...
elseif ($_REQUEST['action'] == "sell") 
{
	...
	$sql="update fjr_shopitems set stock='$newstock', sold='$newsold' where name='{$row['name']}'";
	if ( !($db->sql_query($sql)) )
	{
